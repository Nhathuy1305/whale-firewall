...
DELETE FROM est_containers
WHERE src_container_id = ?
OR dst_container_id = ?;

-- name: DeleteWaitingContainerRules :exec
DELETE FROM waiting_container_rules
WHERE src_container_id = ?;
...
SELECT
    ...
    c.name
FROM containers c
JOIN container_aliases a
ON a.container_id = c.id
WHERE a.container_alias = ?;
...
SELECT
    e.dst_container_id,
    c.name
FROM est_containers e
JOIN containers c
ON c.id = e.dst_container_id
WHERE e.src_container_id = ?;
...
SELECT
    ...
    c.name,
    w.rule
FROM waiting_container_rules w
JOIN containers c
ON c.id = w.src_container_id
WHERE w.dst_container_name = ?;